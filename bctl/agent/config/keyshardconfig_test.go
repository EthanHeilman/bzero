	...
	Context("Adding entries", func() {
		When("Entry does not exist", Ordered, func() {
			...
			BeforeAll(func() {
				...
				By("adding an entry")
				config, _ := LoadKeyShardConfig(mockClient)
		...
		When("Adding many targets at once / different entries", Ordered, func() {
			BeforeAll(func() {
				...
				for i := 1; i <= 4; i++ {
					for j := 1; j <= 12; j++ {
						if j%4 == i {
							go config.AddTarget(data.SplitPrivateKey{D: fmt.Sprintf("%d", i)}, fmt.Sprintf("targetId%d", i))
						}
					}
				}
	...
	Context("Deleting entries", func() {
		...
		When("Deleting many entries at once", Ordered, func() {
			...
			BeforeAll(func() {
				...
				for i := 1; i <= 3; i++ {
					go config.DeleteKey(data.SplitPrivateKey{D: fmt.Sprintf("%d", i)})
				}
